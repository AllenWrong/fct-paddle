...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    with paddle.no_grad():
        ...
        _no_grad_uniform_(tensor, a=2*l-1, b=2*u-1)
        tensor.set_value(paddle.erfinv(tensor))
        new_tensor = tensor * std * math.sqrt(2.)
        new_tensor = new_tensor + mean
        new_tensor = paddle.clip(new_tensor, min=a, max=b)
        tensor.set_value(new_tensor)
        return tensor
